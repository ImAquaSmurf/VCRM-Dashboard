...
<script lang="ts">
    ...
    if (mapInstance) {
      ...
      const markers = leaflet.markerClusterGroup({
        ...
        iconCreateFunction: function (cluster) {
          const color =
            $darkModeStore.value && $colorSchemeStore.value
  ...
  darkModeStore.subscribe(() => {
    localInitializeMap !== undefined && init(localInitializeMap);
  });
...
<LeafletMapLoader {init} darkMode={$darkModeStore.value} class={className} />
